...
# แก้ไขส่วนการจัดการข้อความ: บังคับคอลัมน์ที่ต้องการให้เป็น String และจัดการค่าว่าง
text_cols = ["เครื่องจักร", "กะ", "ลักษณะ เวลาหยุดเครื่อง", "ลักษณะ Order ความยาว", "สาเหตุจาก", "กรุ๊ปปัญหา", "รายละเอียด", "Checked-2"]
for col in text_cols:
    if col in df.columns:
        # แทนที่ค่าว่าง (NaN) ด้วยช่องว่าง และแปลงเป็น String เพื่อป้องกันคำว่า "nan"
        df[col] = df[col].fillna("").astype(str).str.strip()
        # กรองค่าที่ระบบมักสร้างขึ้นเองให้เป็นค่าว่าง
...
def multi_filter(label, col):
    if col in df.columns:
        # กรองรายการที่ว่างออกจากการเลือกใน Sidebar
