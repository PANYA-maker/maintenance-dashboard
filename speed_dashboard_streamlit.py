...
# ======================================
# SECTION: TOP 10 NON-STOP LOSS (MOST NEGATIVE) & INSIGHTS
# ======================================
...
if not non_stop_loss_df.empty:
    # Sort by Diff เวลา ascending (Lowest value = Most negative = Biggest Loss)
    top_10_loss = non_stop_loss_df.sort_values(by="Diff เวลา", ascending=True).head(10)
    
    # Calculate Insights based on Top 10 Losses
    worst_problem_group = top_10_loss["กรุ๊ปปัญหา"].value_counts().idxmax()
    problem_count = top_10_loss["กรุ๊ปปัญหา"].value_counts().max()
    ...
    * **⚠️ ความสูญเสียสะสม:** เฉพาะออเดอร์ 10 รายการนี้ สร้างความสูญเสียเวลารวมสูงถึง **{total_lost_time:,.0f} นาที** ซึ่งเป็นการเสียเวลาโดยที่เครื่องจักรยังรันอยู่
    * **📉 ปัญหาสปีดตก:** มีการทำความเร็วเฉลี่ยต่ำกว่าเป้าหมาย (Speed Plan) ถึง **{avg_speed_drop:,.0f} unit/hr** สะท้อนถึงประสิทธิภาพการเดินงานที่ลดลงอย่างชัดเจน
    * **🏭 คอขวดหลัก:** พบปัญหาในกลุ่ม **"{worst_problem_group}"** บ่อยที่สุด ({problem_count} รายการ) ซึ่งเป็นสาเหตุสำคัญที่ทำให้สปีดไม่เป็นไปตามแผน
    * **📦 กลุ่มงานที่ต้องระวัง:** ลักษณะออเดอร์ประเภท **"{worst_order_type}"** เป็นกลุ่มที่มีความผันผวนของความเร็วสูงสุดและมักจะทำเวลาติดลบมากที่สุด
    * **🔍 ข้อแนะนำ:** ควรเร่งตรวจสอบหาสาเหตุในคอลัมน์ "รายละเอียด" ของรายการเหล่านี้ ว่าเกิดจากทักษะการคุมเครื่อง สภาพวัตถุดิบ หรือการตั้งค่าเครื่องจักรที่ไม่เหมาะสม
    """)
    ...
    # Format to integer for professional look
