...
# 1. จัดการข้อมูลประเภทตัวเลข (เฉพาะคอลัมน์ที่เป็นตัวเลขจริง ๆ)
numeric_cols = ["Speed Plan", "Actual Speed", "เวลา Plan", "เวลา Actual", "เวลาหยุดข้อมูลเครื่อง", "Diff เวลา"]
for col in numeric_cols:
    if col in df.columns:
        df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)

# 2. แก้ไขส่วนที่ตัวหนังสือไม่ขึ้น: บังคับคอลัมน์เป้าหมายให้เป็น String เสมอ
# ป้องกัน pandas เข้าใจผิดว่าเป็นตัวเลขแล้วข้อมูลข้อความหาย
text_target_cols = [
...
for col in text_target_cols:
    if col in df.columns:
        # แปลงเป็น string ก่อนทำอะไรทั้งสิ้น
        df[col] = df[col].astype(str)
        # ล้างค่า 'nan' หรือ 'None' ที่ pandas สร้างขึ้นเมื่อเจอค่าว่าง
        df[col] = df[col].apply(lambda x: "" if x.strip().lower() in ['nan', 'none', 'null'] else x.strip())
...
def multi_filter(label, col):
    if col in df.columns:
        # กรองเอาค่าว่างออกจากการเลือกใน Sidebar
        valid_options = sorted([opt for opt in df[col].unique() if opt != ""])
        return st.sidebar.multiselect(label, valid_options)
    return []
...
if not filtered_df.empty and "วันที่" in filtered_df.columns:
    ...
    def calc_row_overall(row):
        val = 0.0
        if str(row['ลักษณะ เวลาหยุดเครื่อง']).strip() == "ไม่จอดเครื่อง":
            val = row['Diff เวลา']
        elif str(row['ลักษณะ เวลาหยุดเครื่อง']).strip() == "จอดเครื่อง":
