...
if not filtered_df.empty and "วันที่" in filtered_df.columns:
    ...
    def calc_row_overall(row):
        val = 0.0
        # ใช้ Strip เพื่อความแม่นยำในการเช็คเงื่อนไข
...
# Filter "ไม่จอดเครื่อง" โดยใช้ Strip เผื่อมี Space ปน
...
if not non_stop_loss_df.empty:
    # Sort by Diff เวลา ascending (ติดลบมากที่สุดขึ้นก่อน)
    top_10_loss = non_stop_loss_df.sort_values(by="Diff เวลา", ascending=True).head(10)
    
    # คำนวณ Insights พร้อมดัก Error กรณีข้อมูลบางคอลัมน์ไม่มี
    try:
        worst_problem_group = top_10_loss["กรุ๊ปปัญหา"].value_counts().idxmax() if not top_10_loss["กรุ๊ปปัญหา"].empty else "N/A"
        problem_count = top_10_loss["กรุ๊ปปัญหา"].value_counts().max() if not top_10_loss["กรุ๊ปปัญหา"].empty else 0
        worst_order_type = top_10_loss["ลักษณะ Order ความยาว"].value_counts().idxmax() if not top_10_loss["ลักษณะ Order ความยาว"].empty else "N/A"
        ...
        st.error(f"""
        **💡 Executive Insights (สรุปความสูญเสียเชิงลึกสำหรับผู้บริหาร)**
        
        จากการวิเคราะห์ออเดอร์ที่ไม่จอดเครื่องแต่ทำเวลาได้ช้ากว่าแผน (ค่าติดลบมากที่สุด) 10 อันดับแรก พบประเด็นวิกฤตดังนี้:
        
        * **⚠️ ความสูญเสียสะสม:** เฉพาะออเดอร์ 10 รายการนี้ สร้างความสูญเสียเวลารวมสูงถึง **{total_lost_time:,.0f} นาที**
        * **📉 ปัญหาสปีดตก:** มีการทำความเร็วเฉลี่ยต่ำกว่าเป้าหมาย (Speed Plan) ถึง **{avg_speed_drop:,.0f} unit/hr**
        * **🏭 คอขวดหลัก:** พบปัญหาในกลุ่ม **"{worst_problem_group}"** บ่อยที่สุด ({problem_count} รายการ)
        * **📦 กลุ่มงานที่ต้องระวัง:** ลักษณะออเดอร์ประเภท **"{worst_order_type}"** มักจะทำเวลาติดลบมากที่สุด
        * **🔍 ข้อแนะนำ:** ควรเร่งตรวจสอบหาสาเหตุในคอลัมน์ "รายละเอียด" เพื่อหาทางป้องกันเชิงเทคนิค
        """)
    except Exception as e:
        st.warning("ไม่สามารถคำนวณ Insights บางส่วนได้ เนื่องจากข้อมูลไม่ครบถ้วน")
    ...
    st.info("ℹ️ สำหรับเครื่องจักรและช่วงเวลาที่เลือก **ไม่พบออเดอร์ประเภท 'ไม่จอดเครื่อง' ที่ทำเวลาช้ากว่าแผน**")
