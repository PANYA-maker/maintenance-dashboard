...
# =========================
# PDW WEIGHT SUMMARY (IN ISSUE PAGE)
# =========================
...
if pdw_col in fdf.columns:
    ...
    if not pdw_issue_df.empty:

        # แปลงเป็นตัวเลขแบบปลอดภัย
        pdw_issue_df[pdw_col] = pd.to_numeric(
            pdw_issue_df[pdw_col],
            errors="coerce"
        ).fillna(0)

        total_pdw = pdw_issue_df[pdw_col].sum()

        # ===== แสดงผล =====
        st.markdown("### ⚖️ น้ำหนักของเหลือ PDW (รวม)")
        st.metric(
            label="รวมทั้งหมด (เฉพาะงานขาดจำนวน)",
            value=f"{total_pdw:,.2f}"
        )

    else:
        st.info("ไม่มีน้ำหนักของเหลือ PDW สำหรับงานขาดจำนวน")

else:
    st.warning("ไม่พบคอลัมน์ 'น้ำหนักของเหลือ PDW' ในข้อมูล")
