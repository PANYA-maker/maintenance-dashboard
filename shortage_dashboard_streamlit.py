...
with tab2:
    ...
    if "สถานะซ่อมสรุป" in fdf.columns:
        ...
        total_row = pd.DataFrame([{"สถานะซ่อมสรุป": "ผลรวมทั้งหมด", "จำนวนออเดอร์": total_orders, 
                                   "จำนวนเมตรขาดจำนวน": total_meters, "ตารางเมตรขาดจำนวน": total_sqm, 
        ...
        with t1:
            ...
            st.dataframe(issue_df.style.format({"จำนวนออเดอร์": "{:,}", "รวมเมตร (m)": "{:,.0f}", "รวม ตร.ม.": "{:,.0f}", "รวมน้ำหนัก (kg)": "{:,.0f}"}),
