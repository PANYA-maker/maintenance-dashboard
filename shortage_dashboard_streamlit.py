...
# UPDATED: 7 Weeks Default & Sun-Sat Week Cycle
# =====================================
...
with st.sidebar:
    ...
    # คำนวณวันเริ่มต้นย้อนหลัง 7 สัปดาห์ (49 วัน) จากวันล่าสุดในข้อมูล
    # และปรับให้เป็นวันอาทิตย์ต้นสัปดาห์นั้น
    if not pd.isna(max_date):
        seven_weeks_ago = max_date - pd.Timedelta(weeks=7)
        # ปรับให้ไปที่วันอาทิตย์ (Sunday) ของสัปดาห์นั้น (0=Monday, 6=Sunday)
        # สูตร: date - (weekday + 1) % 7
        default_start = seven_weeks_ago - pd.Timedelta(days=(seven_weeks_ago.weekday() + 1) % 7)
    else:
        default_start = None

    date_range = st.date_input("🗓️ เลือกช่วงเวลา (เริ่มต้นที่ 7 สัปดาห์ล่าสุด)",
...
st.markdown(f"""
    <div style="margin-bottom: 25px;">
        ...
        <p style="color:#64748b; font-size:1.1rem;">วิเคราะห์ผลผลิตขาดจำนวน | เริ่มต้น 7 สัปดาห์ล่าสุด (อาทิตย์ - เสาร์)</p>
    </div>
""", unsafe_allow_html=True)
...
if not trend.empty:
    if period == "รายวัน": 
        ...
        # weekday() ของ pandas: Mon=0, Sun=6 -> เพื่อให้ Sun เป็นวันแรก:
        # ใช้สูตร: date - (weekday + 1) % 7
...
st.caption("Shortage Intelligence Dashboard | 7-Week Rolling Window (Sun-Sat) | ข้อมูลครบถ้วน 100%")
