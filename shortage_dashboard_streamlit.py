...
if not trend.empty:
   ...
   # ---------------- SHORTAGE ISSUE SUMMARY (COUNT + PDW WEIGHT) ----------------
st.divider()
...
if all(col in fdf.columns for col in required_cols):
    ...
    if not shortage_df.empty:
        ...
        # ===== สรุปจำนวน + น้ำหนัก PDW =====
        issue_summary = (
            shortage_df
            .groupby("สถานะซ่อมสรุป")
            .agg(
                จำนวน=("สถานะซ่อมสรุป", "count"),
        ...
        st.markdown("### 🔢 น้ำหนักของเหลือ PDW รวมทั้งหมด")
        st.metric(
            label="น้ำหนักคงเหลือ (PDW)",
            value=f"{total_pdw:,.2f}"
        )
    ...
    st.warning("ไม่พบคอลัมน์ที่จำเป็น: สถานะซ่อมสรุป / น้ำหนักของเหลือ PDW")
