...
if all(col in fdf.columns for col in required_cols):

    # เลือกเฉพาะงานขาดจำนวน
    shortage_df = fdf[
        (fdf["สถานะผลิต"] == "ขาดจำนวน") &
        (fdf["สถานะซ่อมสรุป"].notna())
    ].copy()

    # ถ้ามี PDW ให้ใช้ ถ้าไม่มีจะไม่พัง
    has_pdw = "น้ำหนักของเหลือ PDW" in shortage_df.columns
    if has_pdw:
        shortage_df["น้ำหนักของเหลือ PDW"] = pd.to_numeric(
            shortage_df["น้ำหนักของเหลือ PDW"],
            errors="coerce"
        )
    ...
    if not shortage_df.empty:
        ...
        if has_pdw:
            issue_summary = (
                shortage_df
                .groupby("สถานะซ่อมสรุป", as_index=False)
                .agg(
                    จำนวน=("สถานะผลิต", "count"),
                    น้ำหนักของเหลือ_PDW=("น้ำหนักของเหลือ PDW", "sum")
                )
            )
        else:
            issue_summary = (
        ...
        # ✅ sort หลังจากมั่นใจว่ามีคอลัมน์ "จำนวน"
        ...
        with c1:
            ...
            if has_pdw:
                total_pdw = issue_summary["น้ำหนักของเหลือ_PDW"].sum()
                st.metric(
                    "น้ำหนักของเหลือ PDW รวม (งานขาดจำนวน)",
                    f"{total_pdw:,.2f}"
                )
        ...
        with c2:
            fig_issue = px.pie(
                ...
                values="จำนวน",
                hole=0.55,
                title="สัดส่วนปัญหาสถานะซ่อม (งานขาดจำนวน)"
            )
    ...
    st.warning("ไม่พบคอลัมน์ที่จำเป็นสำหรับสรุปสถานะซ่อม")
